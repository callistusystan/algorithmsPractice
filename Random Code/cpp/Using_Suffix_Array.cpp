...
struct suffix_array {
    ...
    auto find(const string& pat, int j=0) { return equal_range(sarray.begin(), sarray.end(), pat, Comp(str,pat.size(),j));}
};
...
int main() {
    ...
    auto p = my_suffix_array.find(my_pattern);
    cout << "find(hell): " << (p.first- my_suffix_array.sarray.begin()) << " " << (p.second- my_suffix_array.sarray.begin()) << endl;
    p = my_suffix_array.find(my_pattern_not_in_sa);
    cout << "find(bob): " << (p.first - my_suffix_array.sarray.begin()) << " " << (p.second- my_suffix_array.sarray.begin())  << endl;
